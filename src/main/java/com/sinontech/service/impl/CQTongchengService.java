...
import com.sinontech.tools.yd.CqYdInterface;
 ...
 *
 */
@Service("CQTongchengService")
public class CQTongchengService implements OrderService {
	...
	public JSONObject sendOrderSms(PageData orderInfo, PageData productInfo) throws Exception {
		...
		String resultStr = CqYdInterface.sendMsg(phone);
		...
		if(code.equals("000000")) {
			String oid = ydResult.getString("data");
			result.put("code", "0");
			result.put("msg", "成功");
			result.put("oid", oid);
		}else {
	...
	public JSONObject orderProduct(PageData orderInfo, PageData productInfo, PageData orderLog) throws Exception {
		...
		if(StringUtils.isBlank(sessionid)) {
			...
			String resultStr = CqYdInterface.submitSmsCode(phone, smsCode,oid);
...
//			orderLog.put("BACK_URL", backUrl);//保存回调地址
	...
	public JSONObject queryUserPackage(PageData queryInfo,PageData productInfo) throws Exception {
		...
		salesCode = salesCode.replace("cq_", "");
